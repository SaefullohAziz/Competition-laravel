	...
	<div class="col-12">
		...
		<div class="card card-primary">
			<div class="card-header">
				<!-- if (auth()->guard('admin')->user()->can('create administrator')) -->
					<a href="{{ route('admin.administrator.create') }}" class="btn btn-icon btn-success" title="{{ __('Create') }}"><i class="fa fa-plus"></i></a>
				<!-- endif -->
            	<button class="btn btn-icon btn-secondary" onclick="reloadTable()" title="{{ __('Refresh') }}"><i class="fa fa-sync"></i></button>
            	<button class="btn btn-icon btn-danger" title="{{ __('File Hapus') }}"><i class="fas fa-trash"></i></button>
			</div>
			<div class="card-body">
		...
		$('[name="deleteData"]').click(function(event) {
			if ($('[name="selectedData[]"]:checked').length > 0) {
				...
				.then((willDelete) => {
					if (willDelete) {
						$.ajax({
							url : "{{ route('admin.administrator.destroy') }}",
							type: "DELETE",
							dataType: "JSON",
